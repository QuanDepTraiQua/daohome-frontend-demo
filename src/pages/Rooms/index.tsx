...
import { getRoomAPI, createRoomAPI, deleteRoomAPI } from "../../services/rooms.service";
...
const Rooms: React.FC = () => {
    ...
    let user: any = { username: 'Admin', role: 'STAFF', first_name: '' };
    try {
    ...
    const fetchRooms = async () => {
        ...
        try {
            ...
            if (filterStatus === 'available') params.is_available = 'True';
            if (filterStatus === 'rented') params.is_available = 'False';
            if (filterDistrict !== 'all') params.district = filterDistrict;
            if (sortPrice !== 'none') params.ordering = sortPrice;

            const result = await getRoomAPI(params);
            ...
            if (result && result.success) {
                ...
                message.error(result?.message || "Lỗi tải danh sách phòng");
            }
        } catch (error) {
            message.error("Lỗi hệ thống khi tải dữ liệu!");
        }
        setLoading(false);
    };
    ...
    useEffect(() => {
        fetchRooms();
    }, [filterStatus, filterDistrict, sortPrice]);
    ...
    const handleCreateRoom = async (values: any) => {
        ...
        formData.append('room_code', `ROOM-${randomId}`);

        Object.keys(values).forEach(key => {
            if (key !== 'images' && values[key] !== undefined && values[key] !== null) {
        ...
        if (result && result.success) {
            ...
            message.error(result?.message || "Thêm phòng thất bại!");
        }
    ...
    const handleDeleteRoom = async (roomId: number) => {
        ...
        if (result && result.success) {
            ...
            message.error(result?.message || "Lỗi khi xóa phòng!");
        }
    };
                ...
                >
                    <Form form={form} layout="vertical" onFinish={handleCreateRoom} initialValues={{ is_available: true, toilet: 'Riêng', pet_policy: 'Không', gate_lock: 'Vân tay' }}>
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={8}>
                                <Form.Item name="house_name" label="Địa chỉ" rules={[{ required: true, message: "Vui lòng nhập địa chỉ" }]}>
                                    <Input placeholder="VD: 1368 Lê Văn Lương" />
                                </Form.Item>
                            </Col>
                            <Col span={8}>
                            ...
                            <Col span={12}>
                                <Form.Item name="pet_policy" label="Quy định nuôi thú cưng">
                                    <Select placeholder="Chọn quy định">
                                        ...
                                        <Option value="Mèo (theo quy định)">Chỉ Mèo</Option>
                                    </Select>
                                </Form.Item>
                            </Col>
                        </Row>
                        ...
                        <div className="modal-footer-actions">
                            <Space>
                                ...
                                <Button type="primary" htmlType="submit" loading={submitLoading} size="large">Lưu dữ liệu phòng</Button>
                            </Space>
                        </div>
                    </Form>
                </Modal>
